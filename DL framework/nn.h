...
Mat mat_alloc(int rows, int cols)
{
    ...
    //? This line of code dynamically allocates memory for the two-dimensional array of floats and assigns the address of the first element of that array to the m.es pointer
    NN_ASSERT(m.es != NULL);
    return m;
}
...
void nn_print(NN nn, const char *name)
{
    ...
    for (int i = 0; i < nn.count; ++i) {
        snprintf(buf, sizeof(buf), "ws%d", i);
        mat_print(nn.ws[i], buf, 4);
        snprintf(buf, sizeof(buf), "bs%d", i);
